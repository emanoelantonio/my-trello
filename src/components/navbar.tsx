            ...
            <div className="flex items-center space-x-2 sm:space-x-4 shrink-0">
              {onFilterClick &&
                ...
                >
                  ...
                  {filterCount > 0 &&
                    <Badge className={`text-xs ml-1 sm:ml-2 bg-blue-100 border-blue-200`} variant='secondary'>
                      {filterCount}
                    </Badge>}
                </Button>}
            </div>
        ...
        <div className="flex items-center space-x-2 sm:space-x-4">
          {isSignedIn ? (
            <div className="flex flex-col sm:flex-row items-end sm:items-center space-y-1 sm:space-y-0 sm:space-x-4">
              <span className="text-xs sm:text-sm text-gray-600 hidden sm:block">Hello, {user?.firstName ?? user?.emailAddresses[0]?.emailAddress}</span>
              <Link href="/dashboard">
